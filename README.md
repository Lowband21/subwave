...
**Relationship to Ferrex**
- subwave is developed primarily to power video playback for Ferrex, a performance-first, self‑hosted media server and desktop player focused on responsive browsing and playback matching the performance and compatibility of native video players. Ferrex emphasizes desktop feel, aiming to be enjoyable to use, pleasant to look at, and to support HDR playback by leveraging bleeding-edge Wayland + GStreamer integration.
...
- Iced fork ‘iced-ferrex’ with wgpu renderer; disable tiny-skia fallback.
